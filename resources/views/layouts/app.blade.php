...
<body class="bg-[#f0f3fb] text-slate-700 flex overflow-hidden h-screen" x-data="{ sidebarOpen: false }">
    <div class="flex h-screen w-full relative">
        ...
        <div class="flex-1 flex flex-col h-screen overflow-hidden">
            ...
            <!-- TOPBAR -->
            <header class="flex items-center justify-between px-6 py-3.5 bg-white border-b border-gray-100 sticky top-0 z-40 shadow-sm">
                <!-- Mobile Menu Toggle -->
                ...
                <div class="flex items-center gap-3 ml-auto">
                    <!-- Theme Toggle -->
                    <button id="darkModeToggle" class="w-9 h-9 flex items-center justify-center bg-gray-50 hover:bg-gray-100 rounded-xl transition">
                    ...
                    <div class="relative" x-data="{ open: false }" @click.away="open = false">
                        <button @click="open = !open" class="flex items-center gap-2 focus:outline-none group">
                            <img class="h-9 w-9 rounded-xl border-2 border-blue-100 group-hover:border-blue-300 transition" src="https://ui-avatars.com/api/?name={{ Auth::user()->name ?? 'User' }}&background=1e2356&color=fff&bold=true" alt="Avatar">
                            <span class="hidden sm:inline text-sm font-semibold text-gray-700 group-hover:text-blue-600 transition">{{ Auth::user()->name ?? 'User' }}</span>
                        ...
                        <div x-show="open"
                             ...
                             x-cloak>
                            <a href="{{ route('admin.profile.edit') }}" wire:navigate class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
                                <i class="fas fa-user-cog mr-3 w-4 text-center text-gray-400"></i> Edit Profile
                            </a>
                            <a href="{{ route('admin.users.index') }}" wire:navigate class="flex items-center px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
            ...
            <footer class="bg-white border-t border-gray-100 px-6 py-3 text-sm text-gray-500 text-center">
                2025 © Surya CMS | <a href="{{ $setting->url ?? '#' }}" class="text-blue-600 hover:underline">{{ $setting->url ?? 'https://suryacms.local' }}</a>
            </footer>
        </div>
    </div>
