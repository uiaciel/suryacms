            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="mb-3 flex gap-2">
                                <button type="button"
                                    class="flex-1 px-4 py-2 border border-blue-500 text-blue-600 rounded-lg hover:bg-blue-50 font-semibold"
                                    wire:click="$dispatch('openGalleryModal')"><i class="fas fa-images mr-2"></i>Insert
                                    Media</button>
                                <button type="button"
                                    class="flex-1 px-4 py-2 border border-red-500 text-red-600 rounded-lg hover:bg-red-50 font-semibold"
                ...
                <div class="space-y-4">
                    <div
                        ...
                        <div class="space-y-4">
                            <div>
                                <label for="datepublish" class="block text-sm font-semibold text-gray-700 mb-2">Date
                                    Publish</label>
                            ...
                            <div
                                ...
                                <button type="button"
                                    ...
                                    <span :class="$wire.feature === 'Yes' ? 'translate-x-5' : 'translate-x-0'"
                            ...
                            <div
                                ...
                                <button type="button"
                                    ...
                                    class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none">
                                    <span :class="$wire.flash === 'Yes' ? 'translate-x-5' : 'translate-x-0'"
                                        class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out"></span>
                                </button>
                            </div>
                    ...
                    <div
                        class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow mb-6">
                        <div class="flex items-center gap-3 mb-4">
                            <div class="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center">
                                <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor"
                                    viewBox="0 0 24 24">
                        ...
                        <div class="space-y-4">
                            <div>
                                <div class="flex items-center justify-between mb-2">
                                    <label for="tags" class="block text-sm font-semibold text-gray-700">Tags /
                                        Keywords</label>
                                    <button type="button"
                                        class="text-xs px-3 py-1 bg-purple-100 text-purple-700 hover:bg-purple-200 rounded-full font-medium transition-colors"
                                        wire:click="generateKeywords" wire:loading.attr="disabled"
                                        wire:loading.class="opacity-50 cursor-not-allowed"
                                        title="Auto-generate keywords from title and content">
                                        <span wire:loading.remove wire:target="generateKeywords"><i
                                                class="fas fa-magic"></i> Auto-Generate</span>
                                        <span wire:loading wire:target="generateKeywords"><i
                                                class="fas fa-spinner animate-spin"></i> Generating...</span>
                                    </button>
                                </div>
                                <input type="text" id="tags" wire:model="tags"
                                    placeholder="Keyword1, Keyword2, Tema, Lainnya"
                                    class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            ...
                            <div x-data="{
                                ...
                                setAlert(detail) { this.alertType = detail.icon || 'info';
                                    this.alertMessage = detail.text || detail.title || 'Info';
                                    this.showAlert = true;
                                ...
                                <label for="source_url" class="block text-sm font-semibold text-gray-700 mb-2">Source
                                    URL</label>
                                <div>
                                    <div class="flex gap-2">
                                        <input x-ref="sourceInput" type="text" id="source_url"
                                            placeholder="https://news.detik.com/berita/judul-berita-dari-url"
                                            wire:model.defer="source_url" name="source_url"
                                        ...
                                        <button type="button"
                                            class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold whitespace-nowrap transition-colors"
                                            x-bind:disabled="$wire.get('source_url') === ''"
                                            wire:click="scrapeContent" wire:loading.attr="disabled"
                                            wire:loading.class="opacity-50 cursor-not-allowed">
                                            <span wire:loading.remove wire:target="scrapeContent"><i
                                                    class="fas fa-search"></i></span>
                                        </button>

                                    </div>
                                    <span wire:loading wire:target="scrapeContent"><i
                                            class="fas fa-spinner animate-spin"></i> Scraping...</span>
                                ...
                                <div class="mt-3">
                                    <template x-if="showAlert">
                                        <div x-bind:class="{
                                            ...
                                            'bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 rounded-lg': alertType === 'info' ||
                                                alertType === 'warning'
                                        }"
                                            role="alert">
                            ...
                            <div hidden>
                                <label for="source_favicon"
                                    class="block text-sm font-semibold text-gray-700 mb-2">Source Favicon</label>
                                <input type="text" id="source_favicon" placeholder="Terisi otomatis"
                                    wire:model="source_favicon" name="source_favicon"
                                    class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>
                            <div hidden>
                                <label for="source_title"
                                    class="block text-sm font-semibold text-gray-700 mb-2">Source Domain</label>
                                <input type="text" id="source_title" placeholder="Terisi otomatis"
                                    wire:model="source_title" name="source_title"
                                    class="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                            </div>

                            <div x-show="$wire.source_title" x-cloak
                                class="mt-3 flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 transition-all animate-fadeIn">
                                <div
                                    class="flex-shrink-0 w-10 h-10 bg-white rounded-lg shadow-sm border border-gray-100 flex items-center justify-center overflow-hidden">
                                    <template x-if="$wire.source_favicon">
                                        <img :src="$wire.source_favicon" class="w-6 h-6 object-contain"
                                            alt="Source Favicon"
                                            onerror="this.onerror=null;this.src='https://www.google.com/s2/favicons?domain='+$wire.source_title;">
                                    </template>
                                    <template x-if="!$wire.source_favicon">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Detected
                                        Source</p>
                                    <p class="text-sm font-bold text-gray-900 truncate" x-text="$wire.source_title">
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="w-full space-y-3">
                        <button type="submit"
                            class="w-full px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold">Publish
                            Post</button>
                        <button type="button" wire:click="saveDraft"
                            class="w-full px-4 py-2 border border-gray-300 text-gray-700 bg-white rounded-lg hover:bg-gray-50 font-semibold">Save
                            Draft</button>
                    </div>
            ...
            <script>
                ...
                function initTinyMCE() {
                    ...
                    tinymce.init({
                        ...
                        image_class_list: [{
                            ...
                            {
                                title: 'Responsive Rounded-4',
                                value: 'img-fluid rounded-4'
                            },
                            {
                                title: 'Responsive Rounded-5',
                                value: 'img-fluid rounded-5'
                            },
                            {
                                title: 'Thumbnail',
                                value: 'img-thumbnail'
                        ...
                        setup: function(editor) {
                            ...
                            editor.ui.registry.addButton('youtubeVideo', {
                                ...
                                editor.on('PastePostProcess', function(e) {
                                    ...
                                    const isLocalPdfPath = pastedText.startsWith('/storage/') && pastedText
                                        .toLowerCase().endsWith('.pdf');
                                    if (isLocalPdfPath) {
                                        const baseUrl = window.location.origin;
                                        const fullUrl = baseUrl + pastedText;
                                        const pdfHtml =
                                            `<div class="pdf-container my-3"><iframe src="${fullUrl}" width="100%" height="600px" frameborder="0" sandbox=""></iframe><p class="mt-2"><a href="${fullUrl}" target="_blank" class="text-blue-600 underline">Download PDF</a></p></div>`;
                                    ...
                                    const isLocalStoragePath = pastedText.startsWith('/storage/') && pastedText
                                        .match(/\.(jpeg|jpg|png|gif|webp)$/i);
                                    if (isLocalStoragePath) {
                                        const baseUrl = "{{ asset('') }}".replace(/\/$/, '');
                                        const fullUrl = baseUrl + pastedText;
                                        const imgTag =
                                        `<img src="${fullUrl}" class="img-fluid" alt="Pasted Image">`;
                                    ...
                                    if (e.clipboardData && e.clipboardData.files.length > 0) {
                                        ...
                                        const isLocalStoragePathFromClipboard = clipboardText.startsWith(
                                                '/storage/') &&
                                        ...
                                        if (isLocalStoragePathFromClipboard && clipboardText !== pastedText) {
                                            ...
                                            editor.execCommand('mceInsertContent', false, imgTag);
                                            e.preventDefault();
                                            console.log("✅ Gambar lokal dimasukkan dari clipboardData teks:",
                                                fullUrl);
                                        }
                                    }
                                });
                            ...
                            editor.on('init change', function() {
                                if (window.selectedImageUrlForTinyMCE) {
                                    this.execCommand('mceInsertContent', false, '<img src="' + window
                                        .selectedImageUrlForTinyMCE +
                                        '" alt="Gambar dari Galeri" class="img-fluid" />');
                                    console.log('Gambar tertunda berhasil disisipkan:', window
                                        .selectedImageUrlForTinyMCE);
                        ...
                        init_instance_callback: function(editor) {
                            ...
                            const target = editor.editorContainer.querySelector(
                                '.tox-toolbar__group button[aria-label="Insert/edit image"]');
                ...
                Livewire.hook('message.processed', (message, component) => {
                    ...
                    if (window.selectedImageUrlForTinyMCE) {
                        const editor = tinymce.get('content');
                        if (editor) {
                            editor.execCommand('mceInsertContent', false, '<img src="' + window.selectedImageUrlForTinyMCE +
                                '" alt="Gambar dari Galeri" class="img-fluid" />');
                ...
                Livewire.on('handle-copy-to-editor', (data) => {
                    ...
                    console.log('💾 Copy to editor event received:', {
                        field,
