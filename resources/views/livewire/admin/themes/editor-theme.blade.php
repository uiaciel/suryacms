<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30 py-6 px-4" x-data="{ showVariableModal: false, mobileSidebar: false, fullscreenEditor: false }">
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.outside="open = false" x-transition:enter="transition ease-out duration-150"
                    ...
                    @foreach ($availableThemes as $theme)
                        <a href="#" wire:click.prevent="changeTheme('{{ $theme }}')" @click="open = false"
    ...
    <div class="flex flex-col lg:flex-row gap-4 items-start relative">
        ...
        {{-- SIDEBAR --}}
        <div
            class="fixed bottom-0 left-0 right-0 z-40 lg:relative lg:bottom-auto lg:w-56 lg:shrink-0 p-4 lg:p-0 bg-white/80 backdrop-blur-md lg:bg-transparent border-t lg:border-t-0 border-slate-200">
            <div
                class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xl lg:shadow-sm lg:sticky lg:top-6">
                    ...
                    @forelse($files as $file)
                        <button wire:click="selectFile('{{ $file['path'] }}')"
                            ...
                            <div class="flex items-center gap-2">
                                <i
                                    class="fas {{ in_array($file['extension'], ['php', 'blade.php']) ? 'fa-code' : 'fa-file-alt' }} text-xs shrink-0
                ...
                <div class="p-3 lg:border-t border-slate-100 bg-slate-50 flex flex-row lg:flex-col gap-2 items-center">
                    <div class="flex-1 lg:hidden">
                        <select
                            class="w-full text-xs border-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
    ...
    {{-- ================================================================= --}}
    @include('suryacms::livewire.admin.themes.variable')
</div>
