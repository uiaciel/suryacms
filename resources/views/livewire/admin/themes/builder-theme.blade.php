<div class="min-h-screen bg-[#f0f3fb] py-6 px-4">
    ...
    @foreach (['message' => 'success', 'error' => 'danger'] as $key => $type)
        @if (session($key))
            <div class="max-w-7xl mx-auto mb-4" x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 5000)">
                <div class="flex items-start gap-3 {{ $type === 'success' ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-rose-50 border-rose-200 text-rose-700' }} border rounded-xl px-4 py-3 shadow-sm">
    ...
    @if (!empty($validationErrors))
        <div class="max-w-7xl mx-auto mb-4" x-data="{ show: true }" x-show="show" x-transition>
            <div class="flex items-start gap-3 bg-amber-50 border border-amber-200 text-amber-700 rounded-xl px-4 py-3 shadow-sm">
    ...
    <div class="max-w-7xl mx-auto">
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                @foreach ($stepNames as $num => $info)
                    <div class="flex items-center gap-3 {{ $step === $num ? 'text-indigo-600 font-semibold' : ($step > $num ? 'text-emerald-600' : 'text-slate-400') }}">
                        <div class="w-9 h-9 rounded-xl flex items-center justify-center text-xs border-2 transition-all duration-300 {{ $step === $num ? 'border-indigo-600 bg-indigo-600 text-white shadow-md' : ($step > $num ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-slate-200 bg-slate-50') }}">
                            @if ($step > $num)
        ...
        <div class="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <div class="relative" x-data="{ uploading: false }" x-on:livewire-upload-start="uploading = true" x-on:livewire-upload-finish="uploading = false" x-on:livewire-upload-error="uploading = false">
                                    ...
                                    <div x-show="uploading" class="mt-2 flex items-center gap-2 text-indigo-600 text-xs font-medium">
            ...
            @if ($step === 2)
                <div class="p-6 space-y-6">
                    <div class="border-b border-slate-100 pb-4">
                        ...
                        <p class="text-slate-400 text-xs mt-1">Klik elemen di kolom kiri untuk melihat di preview (tengah), lalu tentukan selector di kolom kanan.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200">
                        <div>
                            ...
                            @error('selectorNav') <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="selectorFooter" placeholder="#footer atau .footer"
                                   class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 text-xs focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all font-mono">
                            @error('selectorFooter') <p class="text-rose-500 text-[10px] mt-1">{{ $message }}</p> @enderror
                        </div>
                    ...
                    <div class="flex flex-col lg:flex-row gap-4 items-stretch h-[600px]" x-data="{ scrollPos: 0, selectedLine: {{ $selectedIndexItem ?? 0 }} }">
                        ...
                        <div class="w-full lg:w-64 flex flex-col border border-slate-200 rounded-xl bg-white overflow-hidden shadow-sm">
                            <div class="p-3 bg-slate-50 border-b border-slate-200">
                                <p class="text-[11px] font-bold text-slate-600 uppercase tracking-wider">Struktur HTML</p>
                            </div>
                            <div class="flex-1 overflow-y-auto p-2 space-y-1 bg-slate-50/30">
                                <div class="bg-indigo-50/50 border border-indigo-100 rounded-lg p-2 text-slate-600 text-[9px] mb-2">
                                ...
                                @forelse ($htmlStructureIndex as $item)
                                    <button
                                        wire:click="selectIndexItem({{ $item['line'] }})"
                                        ...
                                        <span class="inline-block px-2 py-0.5 rounded text-[9px] font-bold mr-2" :class="{
                                            'bg-blue-100 text-blue-700': '{{ $item['type'] }}' === 'head',
                                            'bg-purple-100 text-purple-700': '{{ $item['type'] }}' === 'header',
                                            'bg-green-100 text-green-700': '{{ $item['type'] }}' === 'nav',
                                            'bg-orange-100 text-orange-700': '{{ $item['type'] }}' === 'section',
                                            'bg-red-100 text-red-700': '{{ $item['type'] }}' === 'footer',
                                        }">
                        ...
                        <div class="flex-1 min-w-0 flex flex-col border border-slate-200 rounded-xl bg-slate-900 overflow-hidden shadow-lg">
                            <div class="p-3 bg-slate-800 border-b border-slate-700 flex justify-between items-center">
                                <p class="text-[11px] font-bold text-slate-300 uppercase tracking-wider">Preview: index.html</p>
                            </div>
                            <div id="codePreview" class="flex-1 overflow-y-auto font-mono text-[11px] leading-relaxed">
                                @if ($htmlOriginalContent)
                                    ...
                                    <table class="w-full">
                                        @foreach ($lines as $lineNum => $line)
                                            <tr class="hover:bg-slate-800 {{ ($lineNum + 1) === $selectedIndexItem ? 'bg-indigo-900/50' : '' }}">
                                                <td class="text-slate-500 select-none px-2 py-0.5 border-r border-slate-700 w-12 text-right">{{ $lineNum + 1 }}</td>
                                                <td class="text-emerald-400 px-3 py-0.5 whitespace-pre-wrap break-words">{!! htmlspecialchars($line) !!}</td>
                                            </tr>
                                        @endforeach
                                    </table>
                                @endif
            ...
            @if ($step === 3)
                <div class="p-6 space-y-6">
                    <div class="border-b border-slate-100 pb-4">
                        ...
                        <p class="text-slate-400 text-xs mt-1">Hubungkan teks statis HTML ke variabel database. Sebelah kanan: preview kode untuk referensi.</p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 h-[600px]" x-data="{ selectedLine: {{ $selectedIndexItem ?? 0 }} }">
                        ...
                        <div class="border border-slate-200 rounded-xl bg-white overflow-hidden flex flex-col">
                            ...
                            <div class="flex-1 overflow-y-auto">
                                ...
                                @else
                                    <table class="w-full text-sm text-left text-slate-500">
                                        <thead class="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-100 sticky top-12 z-10">
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach ($mappings as $index => $map)
                                                <tr data-mapping-row class="bg-white border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                                    <td class="px-4 py-3 text-center">
                                                        <input type="checkbox" data-mapping-checkbox wire:model="mappings.{{ $index }}.enabled"
                                                               class="w-4 h-4 text-indigo-600 bg-white border-slate-300 rounded cursor-pointer">
                                                    </td>
                                                    <td class="px-4 py-3 font-mono text-xs text-slate-700 truncate" title="{{ $map['original'] }}">
                                                        {{ substr($map['original'], 0, 30) }}{{ strlen($map['original']) > 30 ? '...' : '' }}
                                                    </td>
                                                    <td class="px-4 py-3 text-center">
                                                        <span class="inline-block px-2 py-1 rounded text-[10px] font-bold" :class="{
                                                            'bg-blue-100 text-blue-700': '{{ $map['type'] }}' === 'link',
                                                            'bg-purple-100 text-purple-700': '{{ $map['type'] }}' === 'logo',
                                                            'bg-red-100 text-red-700': '{{ $map['type'] }}' === 'email',
                                                            'bg-green-100 text-green-700': '{{ $map['type'] }}' === 'phone',
                                                            'bg-amber-100 text-amber-700': '{{ $map['type'] }}' === 'social',
                                                            'bg-slate-100 text-slate-700': '{{ $map['type'] }}' === 'text',
                                                        }">
                            ...
                            <div class="p-4 bg-slate-50 border-t border-slate-200">
                                <p class="text-xs font-semibold text-slate-600 mb-3 uppercase">Tambah Variabel Manual</p>
                                <div class="space-y-2" x-data="{ newOriginal: '', newReplacement: '', newType: 'text' }">
                                    <input type="text"
                                           x-model="newOriginal"
                                           placeholder="Teks asli di HTML"
                                    ...
                                    <div class="flex gap-2">
                                        ...
                                        <button @click="if(newOriginal.trim() && newReplacement.trim()) { Livewire.dispatch('addManualMapping', {original: newOriginal, replacement: newReplacement, type: newType}); newOriginal = ''; newReplacement = ''; newType = 'text'; }"
                        ...
                        <div class="border border-slate-200 rounded-xl bg-slate-900 overflow-hidden flex flex-col">
                            <div class="p-3 bg-slate-800 border-b border-slate-700 sticky top-0">
                                <p class="text-xs font-semibold text-slate-300 uppercase tracking-wider">Preview: index.html</p>
                            </div>
                            <div id="codePreviewStep3" class="flex-1 overflow-y-auto font-mono text-[10px] leading-relaxed">
                                @if ($htmlOriginalContent)
                                    @php
                                        $lines = explode("\n", $htmlOriginalContent);
                                    @endphp
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="space-y-6" x-data="{ selectedHtmlFile: 'index.html', selectedGenFile: 'navigation.blade.php', loadingCode: false }">
                        ...
                        <div class="flex flex-col lg:flex-row gap-4 h-[400px]">
                            <div class="flex-1 border border-slate-200 rounded-xl bg-slate-900 overflow-hidden flex flex-col">
                                <div class="p-3 bg-slate-800 border-b border-slate-700 flex justify-between items-center">
                                    <p class="text-[11px] font-bold text-slate-300 uppercase tracking-wider">Source Code: <span x-text="selectedHtmlFile" class="text-amber-400"></span></p>
                                    <p class="text-[10px] text-slate-400">Original HTML</p>
                                </div>
                                <div class="flex-1 overflow-y-auto font-mono text-[10px] bg-slate-800 text-emerald-400 p-4 leading-relaxed">
                            ...
                            <div class="w-full lg:w-64 border border-slate-200 rounded-xl bg-white overflow-hidden flex flex-col shadow-sm">
                                <div class="p-3 bg-slate-50 border-b border-slate-200">
                                    <p class="text-[11px] font-bold text-slate-600 uppercase tracking-wider">File HTML</p>
                                </div>
                                <div class="flex-1 overflow-y-auto p-2 space-y-1">
                                    <button @click="selectedHtmlFile = 'index.html'" :class="selectedHtmlFile === 'index.html' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-3 py-2 rounded-lg text-xs font-medium border border-transparent transition-all">📄 index.html</button>
                                    @foreach ($allHtmlFiles as $filename => $filedata)
                                        <button @click="selectedHtmlFile = '{{ $filename }}'" :class="selectedHtmlFile === '{{ $filename }}' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'text-slate-600 hover:bg-slate-50'" class="w-full text-left px-3 py-2 rounded-lg text-xs font-medium border border-transparent transition-all truncate" title="{{ $filename }}">📄 {{ $filename }}</button>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col lg:flex-row gap-4 h-[500px]">
                            <div class="flex-1 border border-slate-200 rounded-xl bg-slate-900 overflow-hidden flex flex-col shadow-lg">
                                <div class="p-3 bg-slate-800 border-b border-slate-700 flex justify-between items-center">
                                    <p class="text-[11px] font-bold text-slate-300 uppercase tracking-wider">Generated: <span x-text="selectedGenFile" class="text-purple-400"></span></p>
                                    <div x-data="{ saved: false, saving: false }" class="flex items-center gap-2">
                                        <div x-show="saving" class="flex items-center gap-1.5 text-[10px] text-slate-400">
                                            <i class="fa-solid fa-circle-notch fa-spin"></i> Saving...
                                        </div>
                                        <button type="button" @click="saving = true; $wire.updateGeneratedFile().then(() => { saved = true; saving = false; setTimeout(() => { saved = false; }, 2000); })" class="px-2.5 py-1 bg-emerald-600/80 hover:bg-emerald-600 text-white text-[10px] font-bold rounded-lg transition-all">
                                            <i class="fa-solid fa-floppy-disk mr-1"></i> <span x-text="saved ? '✓ Saved' : 'Save'"></span>
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="w-full lg:w-64 border border-slate-200 rounded-xl bg-white overflow-hidden flex flex-col shadow-sm">
                                <div class="p-3 bg-slate-50 border-b border-slate-200">
                                    <p class="text-[11px] font-bold text-slate-600 uppercase tracking-wider">Generated Files</p>
                                </div>
                                <div class="flex-1 overflow-y-auto p-2 space-y-1">
                                    @foreach ($generatedFiles as $filename => $content)
                                        <button @click="selectedGenFile = '{{ $filename }}'; loadingCode = true; $wire.loadFileForEditing('{{ $filename }}').then(() => { loadingCode = false; })"
                                                :class="selectedGenFile === '{{ $filename }}' ? 'bg-indigo-50 text-indigo-700 border-indigo-200' : 'text-slate-600 hover:bg-slate-50'"
            ...
            <div class="bg-slate-50 px-6 py-5 border-t border-slate-100 flex items-center justify-between">
                ...
                @else
                    <div class="text-xs text-slate-400 font-medium italic"><i class="fa-solid fa-info-circle mr-1"></i> Lengkapi data untuk lanjut</div>
                @endif
...
<script>
    // Minimize sidebar when builder theme is accessed
    document.addEventListener('DOMContentLoaded', function() {
        // Find the body element with Alpine.js data
        const body = document.querySelector('body');
        if (body && body.__alpine_$data && body.__alpine_$data.sidebarMinimized !== undefined) {
            body.__alpine_$data.sidebarMinimized = true;
        }
    });

    // Also handle Livewire initialization
